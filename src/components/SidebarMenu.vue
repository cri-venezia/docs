<template>
  <ul class="space-y-1 ml-2 border-l border-gray-200">
    <li v-for="item in items" :key="item.path">
      ...
      <div v-if="item.type === 'group'">
        ...
        >
          <span>{{ item.name }}</span>
          <!-- Icona freccia (usa un SVG semplice per evitare dipendenze) -->
...
const props = defineProps({
  items: Array,
  categoryPath: String,
  openGroups: Object // L'oggetto che tiene traccia degli stati (passato da App.vue)
});
